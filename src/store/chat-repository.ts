...
// --- In-memory message cache (moved from chat-store.ts) ---
...
/** In-memory only: normalize + update the memory cache, skip localStorage. */
...
/** Full save: normalize + update memory cache + write to localStorage. */
function saveConversation(
  chatId: string,
  messages: ChatMessage[]
): { normalized: ChatMessage[]; previousCount: number } {
  const { normalized, previousCount } = updateConversation(chatId, messages)
