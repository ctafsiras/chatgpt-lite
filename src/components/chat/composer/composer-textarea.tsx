...
const TEXTAREA_CLASS_NAME =
  'text-foreground w-full min-w-0 resize-none border-0 bg-transparent text-base leading-relaxed break-words outline-none shadow-none focus:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 max-h-52 min-h-6 overflow-y-auto [field-sizing:content]'
  ...
  return (
    <div className="relative flex min-h-11 min-w-0 flex-1 items-start px-4 pt-2 pb-1">
      ...
      {showPlaceholder && !interimTranscript && (
        <span className="text-muted-foreground pointer-events-none absolute top-2 left-4 text-base leading-relaxed">
          Ask anything
        </span>
      )}
      {interimTranscript ? (
